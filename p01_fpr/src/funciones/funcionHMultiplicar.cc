...
int FunctionHMultiplicar::operar(int x, int y) const {
  std::cout << "No se puede realizar la operación" << std::endl;
  exit(1);
}

int FunctionHMultiplicar::operar(std::vector<int> x, int y, int length) const {
  std::cout << "No se puede realizar la operación" << std::endl;
  exit(1);
}

int FunctionHMultiplicar::operar(int x) const {
  std::cout << "No se puede realizar la operación" << std::endl;
  exit(1);
}
...
int FunctionHMultiplicar::operar(int x, int y, FRP* function) const {
  ...
  FRP* function2 = new FunctionSuma();
